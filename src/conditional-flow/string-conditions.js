...
function hasOddNumberVowels(val1) {
  // TODO: write code in this function body to pass the tests
  return  Array.from(val1.toLowerCase()).filter(letter => 'aeiouy'.includes(letter)).length % 2 === 1;
}
...
function getMiddleLetter(val1) {
  // TODO: write code in this function body to pass the tests
  if (!hasOddNumberVowels(val1)) {

    return val1.charAt(val1.length / 2 -1) + val1.charAt((val1.length / 2));
  } else return val1.charAt(Math.floor(val1.length / 2));
}
